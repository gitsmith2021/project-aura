...
import { requireSuperAdmin } from "@/actions/superAdmin";
...
export async function resetDemoInstitution(): Promise<Result<{ institutionId: string; cleared: true }>> {
  try {
    // ── Guard 1: SUPER_ADMIN, DB-revalidated — reuses the same helper the rest of
    //    /admin uses (Phase 7B), never trusting the aura-role cookie. ──
    const userId = await requireSuperAdmin();
    if (!userId) return { success: false, error: "Only a SUPER_ADMIN may reset the demo tenant." };
    ...
    await logAudit({
      institutionId: I, performedBy: userId, tableName: "institutions", recordId: I,
