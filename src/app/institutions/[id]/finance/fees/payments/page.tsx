...
export default async function FeePaymentsPage({ params }: PageProps) {
  ...
  const [{ data: institutions }, { data: institution }, { data: feeStructures }] = await Promise.all([
    supabase.from("institutions").select("id, name").order("name"),
    supabase.from("institutions").select("name").eq("id", id).single(),
    supabase.from("fee_structures").select("id, name, amount").eq("institution_id", id).eq("is_active", true).order("name"),
  ]);
  ...
  const breadcrumb = (
    <>
      ...
      <span className="mx-2 text-slate-300 dark:text-slate-600">/</span>
      <Link href={`/institutions/${id}/finance/fees`} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors">Fee Structures</Link>
      <span className="mx-2 text-slate-300 dark:text-slate-600">/</span>
