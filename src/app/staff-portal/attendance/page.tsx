...
import { getStaffProfile, getStaffAttendanceSummary } from "@/actions/staffPortal";
...
export default async function AttendancePage() {
  ...
  const summaryResult = await getStaffAttendanceSummary(staff.id);
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100/80 dark:border-slate-700/60">
                {["Subject","Day & Time","Classes Held","Present","Total","Attendance %"].map(h => (
              ...
              {rows.map(r => (
                <tr key={r.schedule_id} className="hover:bg-slate-50/60 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 whitespace-nowrap">
                    {r.day_of_week} · {fmtTime(r.start_time)}–{fmtTime(r.end_time)}
                  </td>
