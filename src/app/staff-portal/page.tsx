...
import { Calendar, Users, ClipboardCheck, Clock, BookOpen, Building2, Wallet, Briefcase } from "lucide-react";
import Link from "next/link";
import { getStaffProfile, getStaffDashboardStats, getLeaveRequests } from "@/actions/staffPortal";
import { isNonTeaching, isWarden, isDailyWage, staffTypeLabel, type StaffType } from "@/lib/staffTypes";
...
export default async function StaffDashboard() {
  ...
  const staffType = (staff.staff_type ?? "teaching") as StaffType;
  const nonTeaching = isNonTeaching(staffType);
    ...
    <div className="px-6 pt-4 pb-6 space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          {nonTeaching && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-900/25 dark:text-blue-300 dark:border-blue-800/40">
              <Briefcase size={9} />
      ...
      {/* KPI cards — teaching staff see class/student counts; non-teaching see role-relevant KPIs */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        {!nonTeaching ? (
          ...
          <>
            <div className={`${cardCls} border-blue-200/60 dark:border-blue-800/40 col-span-2 xl:col-span-2`}>
              <div className="flex items-center gap-2 mb-1">
                <Briefcase size={13} className="text-blue-500" />
                <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Role</p>
              </div>
              <p className="text-sm font-bold text-blue-700 dark:text-blue-400">{staffTypeLabel(staffType)}</p>
              <p className="text-[10px] text-slate-400 mt-0.5">{staff.designation ?? "Non-teaching staff"}</p>
            </div>
          </>
        )}
      ...
      {/* Daily-wage info banner */}
      {isDailyWage(staffType) && staff.daily_wage_rate != null && (
        <div className="rounded-xl bg-emerald-50/80 dark:bg-emerald-900/20 border border-emerald-200/60 dark:border-emerald-800/40 px-4 py-3 flex items-center gap-3">
          <Wallet size={16} className="text-emerald-600 shrink-0" />
          <div>
            <p className="text-xs font-semibold text-emerald-800 dark:text-emerald-300">Daily Wage Rate</p>
            <p className="text-[11px] text-emerald-700 dark:text-emerald-400">
              ₹{Number(staff.daily_wage_rate).toLocaleString("en-IN", { minimumFractionDigits: 2 })} per working day attended.
              View your monthly wage slips under{" "}
              <Link href="/staff-portal/salary" className="underline font-medium hover:text-emerald-900">
                My Salary
              </Link>.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Teaching: show timetable; Non-teaching: show role-specific quick links */}
        {!nonTeaching ? (
          ...
          <div className="rounded-xl bg-white/70 dark:bg-slate-800/60 border border-white/40 dark:border-slate-700/50 backdrop-blur-sm shadow-sm">
            <div className="px-4 py-3 border-b border-slate-100/60 dark:border-slate-700/40">
              <h2 className="text-xs font-semibold text-slate-700 dark:text-slate-200">Quick Links</h2>
            </div>
            <div className="px-4 py-3 space-y-2">
              {isWarden(staffType) && (
                <Link
                  href="/staff-portal/hostel"
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-amber-50/60 dark:bg-amber-900/15 border border-amber-100/60 dark:border-amber-800/30 hover:bg-amber-100/60 transition-colors group"
                >
                  <Building2 size={15} className="text-amber-600 shrink-0" />
                  <div>
                    <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 group-hover:text-amber-700">Hostel Management</p>
                    <p className="text-[10px] text-slate-400">View rooms, residents and complaints</p>
                  </div>
                </Link>
              )}
              <Link
                href="/staff-portal/salary"
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-violet-50/60 dark:bg-violet-900/15 border border-violet-100/60 dark:border-violet-800/30 hover:bg-violet-100/60 transition-colors group"
              >
                <Wallet size={15} className="text-violet-600 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 group-hover:text-violet-700">
                    {isDailyWage(staffType) ? "My Wage Slips" : "My Salary"}
                  </p>
                  <p className="text-[10px] text-slate-400">
                    {isDailyWage(staffType)
                      ? "View monthly wage disbursements"
                      : "View pay slips and salary structure"}
                  </p>
                </div>
              </Link>
              <Link
                href="/staff-portal/leaves"
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-blue-50/60 dark:bg-blue-900/15 border border-blue-100/60 dark:border-blue-800/30 hover:bg-blue-100/60 transition-colors group"
              >
                <Calendar size={15} className="text-blue-600 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 group-hover:text-blue-700">Leave Requests</p>
                  <p className="text-[10px] text-slate-400">Apply for leave or check status</p>
                </div>
              </Link>
            </div>
          </div>
        ...
        {/* Recent leaves — shown for all staff types */}
        <div className="rounded-xl bg-white/70 dark:bg-slate-800/60 border border-white/40 dark:border-slate-700/50 backdrop-blur-sm shadow-sm">
          <div className="px-4 py-3 border-b border-slate-100/60 dark:border-slate-700/40">
