...
import { getStudentProfile, getStudentDashboardStats } from "@/actions/studentPortal";
    ...
    <div className="px-6 pt-4 pb-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          {student.student_program ?? "Student"} · Year {student.student_year ?? "—"} · {student.departments?.name ?? ""} · {student.institutions?.name ?? ""}
        </p>
      </div>
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200/60 dark:border-amber-800/40">
          ...
          <div>
            ...
            <p className="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5">
              Your overall attendance is {stats.overallAttendancePct}%. You risk being barred from exams. Attend more classes.
            </p>
          </div>
        </div>
      )}
