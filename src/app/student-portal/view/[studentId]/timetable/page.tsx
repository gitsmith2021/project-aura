...
import { getStudentTimetable }   from "@/actions/studentPortal";
...
export default async function AdminStudentTimetable({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const { data: student } = await supabase
    .from("students")
    .select("id, full_name, department_id, student_program, student_year, departments(name)")
    .eq("id", studentId).single();

  if (!student) redirect("/users/students");

  const dept = (student.departments as unknown as { name: string } | null)?.name ?? "";
  const result = await getStudentTimetable(student.department_id ?? "");
    ...
    <div className="px-6 pt-4 pb-6 space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          {dept} · {student.student_program ?? ""} Year {student.student_year ?? ""}
        </p>
      </div>
