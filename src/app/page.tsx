  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-10 h-10 rounded-xl bg-violet-600 flex items-center justify-center border border-violet-500 shadow-lg shadow-violet-500/30">
...
function Dashboard() {
  ...
  return (
    <DashboardLayout>
      <div className="w-full h-[calc(100vh-56px)] min-h-0 flex flex-col overflow-hidden">
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          {loading ? (
            ...
            <CollegeDashboard college={activeCollege} onViewReport={setSelectedScheduleId} />
          ) : colleges.length === 0 ? (
              ...
              <div className="max-w-md w-full bg-white/70 dark:bg-slate-800/60 border border-white/40 dark:border-slate-700/50 backdrop-blur-md rounded-2xl p-8 shadow-xl text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed max-w-sm mx-auto">
                    Create your first academic institution to start managing departments, staff, students, schedules, and financial operations.
                  </p>
                </div>
                <button type="button" onClick={() => setAddOpen(true)}
                  className="mx-auto inline-flex items-center gap-1.5 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl border border-indigo-700 shadow-md transition-all hover:scale-[1.02] active:scale-[0.98]">
                  <Plus size={14} strokeWidth={2.5} /> Add Institution
                </button>
              </div>
            </div>
          ) : (
      ...
      {selectedScheduleId && (
        <SessionSummaryModal scheduleId={selectedScheduleId} onClose={() => setSelectedScheduleId(null)} />
      )}
      {addOpen && (
        <AddInstitutionModal isOpen={addOpen} onClose={() => setAddOpen(false)} onSuccess={fetchColleges} />
      )}
    </DashboardLayout>
  );
}
