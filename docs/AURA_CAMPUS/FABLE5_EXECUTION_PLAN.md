...
| Phase 8 — Sprint 2 (P8.1 mobile screens) | Sonnet 4.6 | ✅ No new schema. Student **Examination Results** (`exam_results` marksheet, CGPA + per-sem grades/GP/arrears), **Profile** (role-aware identity), **Knowledge Hub** (`study_materials`, grouped by subject), **Downloads** (`certificate_requests` status + web-open), shared **Notifications** inbox (`notifications` read + mark-read, pull-to-refresh) — all read directly under RLS; staff **CIA Marks Entry** (component list → roster marks grid) with the Save **write** via a new authenticated **`POST /api/staff/cia-marks`** + `src/lib/staffMobileAuth.ts` (mirrors `/api/parent`; token-scoped RLS write enforcing the CF-1 `faculty_portal.marks_entry` gate + audit log, never bypassed); middleware exempts `/api/staff` Bearer auth; secondary screens reached via Home/Account quick-links (off the bottom tab bar). Expo Go, web build + mobile typecheck green | `<this commit>` |
...
| IQAC & Government Compliance (7F) | Opus 4.8 | ✅ iqac_meetings + iqac_action_items + RLS; lib NAAC 6.1 compliance (>=2/yr) + action resolved-%/overdue + criterion banding (8 tests); IQAC dashboard (criterion rings via reused SSR aggregator + meeting/action health), Meeting & Action tracker (agenda/minutes editor/action items), year-scoped printable AQAR; NAAC/NIRF/AISHE reused from 7F-sub SSR (not duplicated); Sidebar + dataRetention. **Phase 7 build complete (7A–7F + SSR); Knowledge Hub → 7X.** | `3cc2876` |
...
*Fable 5 free window active until June 22, 2026 — running well ahead of schedule. All 10 Tier‑1 web/data modules complete; mobile (module 11) built on Opus — Phase 8A + 8B/8C + 8F parent-app foundation built (Expo Go); NFC/push/CCTV/in-app-pay remain (need EAS/device).*
